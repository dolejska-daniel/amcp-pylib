...
class CoreSyntaxParserTestCase(unittest.TestCase):
    ...
    def test_get_token(self):
        sources = [
            ...
            ("CLEAR_ON_404", TokenType.KEYWORD),
        ...
        for source, source_token_type in sources:
            with self.subTest():
                p = Parser(Scanner(source))
                t = p.get_token()
                self.assertEqual(t.get_type(), source_token_type)
                self.assertEqual(t.get_content(), source)

    def test_get_token_of_type(self):
        sources = [
            ("identifier", TokenType.IDENTIFIER, TokenType.IDENTIFIER),
            ("ints", TokenType.IDENTIFIER, [TokenType.IDENTIFIER]),
            ("strings", TokenType.IDENTIFIER, [TokenType.KEYWORD, TokenType.IDENTIFIER]),
            ("floats", TokenType.IDENTIFIER, []),
            ("floats", TokenType.IDENTIFIER, None),
            ("int", TokenType.TYPE, TokenType.TYPE),
            ("string", TokenType.TYPE, [TokenType.TYPE]),
            ("float", TokenType.TYPE, [TokenType.KEYWORD, TokenType.IDENTIFIER, TokenType.TYPE]),
            ("raw", TokenType.TYPE, TokenType.TYPE),
            ("float", TokenType.TYPE, []),
            ("float", TokenType.TYPE, None),
            ("-", TokenType.CONSTANT, TokenType.CONSTANT),
            ("1", TokenType.CONSTANT, [TokenType.CONSTANT]),
            ("10", TokenType.CONSTANT, [TokenType.KEYWORD, TokenType.IDENTIFIER, TokenType.CONSTANT]),
            (" ", TokenType.CONSTANT_SPACE, TokenType.CONSTANT_SPACE),
            (" ", TokenType.CONSTANT_SPACE, TokenType.CONSTANT),
            ("KEYWORD", TokenType.KEYWORD, TokenType.KEYWORD),
            ("KEY_WORD", TokenType.KEYWORD, TokenType.CONSTANT),
            ("CLEAR_ON_404", TokenType.KEYWORD, TokenType.KEYWORD),
            ("[", TokenType.REQUIRED_OPEN, TokenType.REQUIRED_OPEN),
            ("]", TokenType.REQUIRED_CLOSE, TokenType.REQUIRED_CLOSE),
            ("{", TokenType.OPTIONAL_OPEN, TokenType.OPTIONAL_OPEN),
            ("}", TokenType.OPTIONAL_CLOSE, TokenType.OPTIONAL_CLOSE),
            ("|", TokenType.OPERATOR_OR, TokenType.OPERATOR_OR),
            (":", TokenType.OPERATOR_TYPE, TokenType.OPERATOR_TYPE),
            (",", TokenType.OPERATOR_COMMA, TokenType.OPERATOR_COMMA),
            ("$", TokenType.UNDEFINED, TokenType.UNDEFINED),
            ("#", TokenType.UNDEFINED, TokenType.UNDEFINED),
        ]
        for source, source_token_type, requested_token_type in sources:
            with self.subTest():
                p = Parser(Scanner(source))

                if not requested_token_type or source_token_type == requested_token_type \
                        or (isinstance(requested_token_type, list) and source_token_type in requested_token_type):
                    t = p.get_token(token_type=requested_token_type)
                    self.assertEqual(t.get_type(), source_token_type)
                    self.assertEqual(t.get_content(), source)

                else:
                    self.assertRaises(RuntimeError, p.get_token, token_type=requested_token_type)

    def test_try_get_token(self):
        sources = [
            ("identifier", TokenType.IDENTIFIER, TokenType.IDENTIFIER),
            ("ints", TokenType.IDENTIFIER, [TokenType.IDENTIFIER]),
            ("strings", TokenType.IDENTIFIER, [TokenType.KEYWORD, TokenType.IDENTIFIER]),
        ...
        for source, source_token_type, requested_token_type in sources:
            with self.subTest():
                p = Parser(Scanner(source))
                t = p.try_get_token(token_type=requested_token_type)

                print(requested_token_type, t)
                if not requested_token_type or source_token_type == requested_token_type \
                        or (isinstance(requested_token_type, list) and source_token_type in requested_token_type):
                    self.assertEqual(t.get_type(), source_token_type)
                    self.assertEqual(t.get_content(), source)
                    ...
                    self.assertFalse(t)
                    t = p.try_get_token(token_type=source_token_type)
                    self.assertEqual(t.get_type(), source_token_type)
                    self.assertEqual(t.get_content(), source)
